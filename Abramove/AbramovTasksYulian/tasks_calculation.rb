...
module TasksCalculation
  def self.task_182(sequence_numbers)
    result_sequence = CalculationMethods.get_numbers_devided_by_five_and_not_seven(sequence_numbers)
    {
  ...
  def self.task_323(natural_number)
    result_sequence = (1..natural_number).find_all do |el|
      (CalculationMethods.get_devides_sequence(el) & CalculationMethods.get_devides_sequence(natural_number)).max == 1
    end
  ...
  def self.task_560(_first_end_numbers)
    ...
    (_first_end_numbers[0].._first_end_numbers[1] - 1).each do |i|
      (i + 1.._first_end_numbers[1]).each do |j|
        sum_devides_by_i = Array(CalculationMethods.get_devides_sequence(i).first(CalculationMethods.get_devides_sequence(i).size - 1)).inject(0, :+)
        sum_devides_by_j = Array(CalculationMethods.get_devides_sequence(j).first(CalculationMethods.get_devides_sequence(j).size - 1)).inject(0, :+)
